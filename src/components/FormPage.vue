  ...
  <section class="form-section">
    <div class="form-section__content">
      <form class="form" @submit.prevent="submit">
        <a @click="formModal.hideModal()" class="formBack univer__link"><img style="transform: rotate(180deg)" src="../assets/sectionArrow.svg" alt="arrow">Назад</a>
        <div class="form__logo"></div>
        <h3 class="form__title">Оставить заявку</h3>

        <input :class="{'error-border': error && model.name === ''}" type="text" class="form__input" placeholder="Имя*" v-model="model.name">
        <label class="input-tel__label">
          <div class="input-tel__prefix">+7</div>
          <input :class="{'error-border': error && model.phone.length < 10}" class="form__input input-tel" style="margin: 0;" type="tel" v-mask="'(000) 000-00-00'" @accept="onAccept" :value="model.phone">
        </label>
